...
nrofvolsskip = 0;%if data discarding dummy scans it should keep 0
imgx = 64;
imgy = 64;
pdimx = 3;
pdimy = 3;
...
SD3DSS=1;   % step 3: Spatial Gaussian Smoothing
THPGLM=1;   % step 4: Temporal High Pass Filter
...
rawdir = {
    'F:\raw_data\XXX_XX\Unnamed - 1566'
    };
rundir={
    'FFA_LOC_1'
    'FFA_LOC_2'
};
% set the output folders and name
parent_dir = 'E:\sample';
fmr_dir = 'bvqx';
fmr_file={
    'loc1'
...
anatdir = 'SAG_FSPGR_BRAVO_ASSET_3';
...
%     subjdir=fullfile(rawdir, subject{i}, uf_tmp{1});
    ...
    fprintf(1,'Raw data in %s \n', raw_dir );
    % creating sub-folder for each subject
    if ~exist(parent_dir, 'dir')
        mkdir(parent_dir);  cd(parent_dir);
    else
        cd(parent_dir);
    end
    mkdir([parent_dir '\' id]);mkdir([parent_dir '\' id '\' fmr_dir]);
...
%             if ~exist(fullfile(subjdir, crundir),'dir') %check folder existed
            ...
            bvqx.RenameDicomFilesInDirectory(fullfile(raw_dir, crundir))
        end
    end
    
    to_be_aligned_fmr_name = strcat(fullfile(parent_dir, id, fmr_dir), '\', id, '_', fmr_file{1}, '_firstvol.fmr');
    % to_be_aligned_fmr_name=strcat(fullfile(subjdir, fmr_dir), '\', 'Vote_1', '_firstvol.fmr');
    yy=[];
    for j=1:length(rundir)
        cd(raw_dir);
        % creating fmr project
...
%         if ~exist(fullfile(subjdir, crundir),'dir') %check folder existed
        ...
        f=dir(fullfile(raw_dir, crundir));
        f = regexpi({f.name},'.*00001.dcm','match'); f=[f{:}]; f=f{1};
        first_dcm_names=strcat(raw_dir,'\',crundir, '\', f);
...
%         fmrname = 'D:\bvqx_stuff\meng_testing\r01.fmr';%strcat('D:\bvqx_stuff\meng_testing\','r',num2str(j,'%02d'), '.fmr');
        fmrname = strcat(fullfile(parent_dir, id, fmr_dir), '\', id, '_', fmr_file{j}, '.fmr');
...
%         pathstr='D:\bvqx_stuff\meng_testing';
        ...
        fmr.SaveAs(fmrname);
        fmr.Close;
        cd(fullfile(parent_dir, id, fmr_dir));
...
%         docFMR.LinkStimulationProtocol(strcat(fullfile(parentdir), '\', prt_names{j,1}));
        ...
        [y(:,1), y(:,2), y(:,3), y(:,4), y(:,5), y(:,6)]=textread([fullfile(parent_dir, id, fmr_dir) '\' id, '_', fmr_file{j} '_SCLAI_3DMC.sdm'],'%f%f%f%f%f%f','headerlines',9);
        yy=[yy;y];
        end
        if SD3DSS
        docFMR = bvqx.OpenDocument( ResultFileName );
    ...
    if triDMC
        ...
        title(subject{i});
        print('-dpng',[fullfile(parent_dir, id, fmr_dir) '\' 'realign.png'])
        close
    end
end%subject
fprintf(1,'Done\n' );